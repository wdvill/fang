...
<?php if(is_array($user_groups)): ?><?php $i = 0;?><?php $__LIST__ = $user_groups?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li id="user_group_<?php echo ($vo['user_group_id']); ?>">
		<div class="c1">
			<div>
				<label><input type="radio" name="user_group_id" value='<?php echo ($vo['user_group_id']); ?>' <?php if($selected[0] == $vo['user_group_id']) { echo 'checked="checked"'; } ?>>
				<span class="user_group_name"><?php echo ($vo['title']); ?></span></label>
			</div>
		</div>
