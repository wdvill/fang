...
class UserCountModel extends Model {
	...
	public function setUserFollowingCount($uids)
	{
		...
		$uids = implode(',', $uids);
		$sql = "SELECT count(*) AS `count`, `uid` FROM {$this->tablePrefix}follow WHERE `uid` IN ( {$uids} ) AND `type` = 0 GROUP BY `uid`";
		$res = M('follow')->query($sql);
	...
	public function setUserFollowerCount($uids)
	{
		...
		$uids = implode(',', $uids);
		$sql = "SELECT count(*) AS `count`, `fid` FROM {$this->tablePrefix}follow WHERE `fid` IN ( {$uids} ) AND `type` = 0 GROUP BY `fid`";
		$res = M('follow')->query($sql);
	...
	//获取用户群组数量
	function getUserGroupCount($uid){
		return M("group_member")->field('gid')->where("uid={$uid} AND level>0")->count();
	}
	
	//获取用户活动数量
	function getUserEventCount($uid){
		return M("event_user")->field('id')->where("uid={$uid} AND (action='admin' OR action='joinIn')")->count();
	}
	
	//获取用户问答数量
	function getUserQaCount($uid){
		return M("question")->field('ques_id')->where("uid={$uid} AND status=1")->count();
	}
	
	//获取用户博客数量
	function getUserBlogCount($uid){
		return M("blog")->field('id')->where("uid={$uid} AND status=1")->count();
	}
}