...
class SearchService extends Service {
	...
	/* Sphinx搜索查询服务 */
	public function Query ( $query, $index="*", $comment="",$seg=true )
	{
		if($seg) $query	=	$this->parseQuery($query);
	...
	public function parseQuery($text) {
		$words	=	$this->getQueryWords($text,5);
		$outwords	=	implode('|',$words);
		return $outwords;
	}

	//获取解析过的词
	public function getQueryWords($text,$limit=5) {
		$text	=	str_replace(' ',',',$text);
		$tag	=	X('Tag');
		$tag->setText($text);
		$words	=	$tag->getTop(3);
		$outwords	=	explode(',',$text.','.$words);
		$outwords	=	array_unique($outwords);
		$outwords	=	array_filter($outwords);
		return $outwords;
	}
	/* 后台管理相关方法 */
