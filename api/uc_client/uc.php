...
class uc_note {
	...
	function face($get){
		if($get['type'] !== "face"){
			$uc_uid = $get['uid'];
			$uc_user_ref = get_ucenter_user_ref('',$uc_uid);
			$user = M('user')->where("uid={$uc_user_ref['uid']}")->find();
			if($user) {
				echo $user['uid'];
				/*cookie('LOGGED_USER',base64_encode('thinksns.'.$user['uid']),(3600*2))*/;
			}
		}else{
	...
	function renameuser($get, $post) {
		...
		update_ucenter_user_ref('',$uc_uid,$usernamenew);
		return API_RETURN_SUCCEED;
	}
	...
	function synlogin($get, $post) {
		...
		if($user) {
			...
			if($get['username'] != $uc_user_ref['uc_username']){
				update_ucenter_user_ref($uc_user_ref['uid'],$uc_uid,$get['username']);
			}
			//成功登录后，设置Cookie
			//$remember ? 
			cookie('LOGGED_USER',base64_encode('thinksns.'.$user['uid']),(3600*24*365)) //: 
			/*cookie('LOGGED_USER',base64_encode('thinksns.'.$user['uid']),(3600*2))*/;
	...
	function updatepw($get, $post) {
		...
		$uc_user_ref = get_ucenter_user_ref('','',$uc_username);
