...
class ContentAction extends AdministratorAction {
	...
	public function doEditAboutus() {
		...
		if($res) {
			if( !isset($_POST['info_id']) ) {
				$this->assign('jumpUrl', U('admin/Content/editAboutus'));
			}else {
				$this->assign('jumpUrl', U('admin/Content/editAboutus'));
			}
			$this->success('保存成功');
	...
	public function doEditSubinfo() {
		...
		// 提交数据
		$res = isset($_POST['id']) ? M('ts_sub_info')->save($_POST) : M('ts_sub_info')->add($_POST);
	...
	public function info() {
		...
		$areas = model('Area')->getAreaList(2);
		$cus_areas = array();
	...
	public function doEditInfo() {
		...
		$_POST['opening_time']	= intval(t($_POST['opening_time']));
