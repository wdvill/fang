...
class ToolAction extends AdministratorAction {
	var $tools;
	var $site_pre = 'http://www.tsqss.com.cn';
	...
	function device(){
		...
		$data['list'] = $this->tools->getDeviceList('status=1 and type="ios"');
	...
	function androiddevice(){
		...
		$data['list'] = $this->tools->getDeviceList('status=1 and type="android"');
	...
	function ioslist(){
		...
		$map = ' send_type="ios"';
		$order = ' id DESC ';
		$data['list'] = M('push_message')->where($map)->order($order)->findPage(10);
	...
	function doadddevice(){
		echo $this->tools->addDevice($_POST);
	}
	
	//删除ios设备
	function deletedevice(){
		echo $this->tools->deleteDevice($_POST['id']);
	}
	
	//删除ios设备2
	function deletedevice2(){
		if( empty($_POST['id']) ) {
			echo 0;
			exit ;
		}
		$ids = is_array($_POST['id']) ? implode(',',$_POST['id']):$_POST['id'];
		$map['id'] = array('in', t($ids));
		echo M('push_message')->where($map)->delete() ? '1' : '0';
	}
	...
	function dosendpush(){
		...
		$url = trim($_POST['url']);
		if (!stristr($url,'http://') && $content_type == '1'){
			$id = $url;
			$url = $this->site_pre.'/info/'.$url;
		} else {
			$id = $url;
		}
		
		if(empty($token) || $token=='all'){
			$map = ' status=1 and type="ios"';
			$deviceToken = $this->tools->getAllDevice($map);
		}else{
			$deviceToken = is_array($token) ? $token : explode(',',$token);
		}
		
		echo $this->tools->sendPush($deviceToken,$title,$id,$url,$content_type);
	}
	
	function dosendandroidpush(){
		...
		if (!stristr($url,'http://') && $content_type == 1){
			$data['url'] = $this->site_pre.'/info/'.$url;
		} else {
			$data['url'] = $url;
		}
		$data['content_sound'] = 'default';
		$data['send_type'] = 'android';
		
		
		if(M('push_message')->add($data)){
				echo '1';
		} else {
				echo '插入数据库失败，请重试！';
		}
