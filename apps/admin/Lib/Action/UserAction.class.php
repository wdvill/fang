...
class UserAction extends AdministratorAction {
    ...
    public function user() {
    	...
    	if ( $_SESSION['isadminman']) {
    		$map = '';
    	} elseif ( $_SESSION['issecondadmin']) {
    		$map = "pid={$_SESSION['mid']}";
    	} else {
    		exit("error");
    	}

    	$res = $dao->getUserList($map, true, true);
    	$this->assign($res);
        $this->display();
    }
    ...
    public function doAddUser() {
		...
		if ( $_SESSION['issecondadmin']) {
			$pid = $_SESSION['mid'];
			$res = M('user')->where("uid={$uid}")->setField(array('pid'), array($pid));
		} elseif ( $_SESSION['isadminman'] && $_POST['user_group_id'] == 12) {
			$pid = $uid;
			$res = M('user')->where("uid={$uid}")->setField(array('pid'), array($pid));
		} else {
...
//     	exit;
    ...
    public function doEditUser() {
		...
		if($_FILES['imgfile']) {
			...
			$head_pic_url = str_replace(SITE_PATH, '', $pic_path);
		}		
    ...
    public function doSearchUser() {
    	...
    	if( $_SESSION['isadminman']) {
	    	if ( isset($_POST['pname']) && $_POST['pname'] != '' ) {
	    		$pids = array();
	    		$pusers = M('user')->field("`uid`")->where('uname LIKE "%'.$_POST['pname'].'%"')->findall();
    ...
    public function verified()
    {
    	...
    	$verified = (1 == $_GET['verified']) ? $_GET['verified'] : 0;
