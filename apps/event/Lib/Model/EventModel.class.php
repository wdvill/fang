...
class EventModel extends BaseModel{
	...
	public function getEventList($map='',$order='id DESC',$mid){
        ...
        if( !empty( $result['data'] )){
            ...
            foreach( $result['data'] as &$value ){
				...
				$value['url'] = $this->getEventUrl('/index/eventDetail',array('id'=>$value['id'],'uid'=>$value['uid']));
				$value['intro'] = msubstr(real_strip_tags($value['explain']),0,60);
	...
	public function appendContent( $data ){
		...
		if( $result = $userDao->hasUser( $this->mid,$data['id'],'joinIn' ) ){
			...
			return $data;
		}else if($result = $userDao->hasUser( $this->mid,$data['id'],'attention' ) ){
			$data['canAtt'] = false;
		}else if($result = $userDao->hasUser( $this->mid,$data['id'],'verify' ) ){
			$data['canAtt'] = false;
			$data['verify'] = $result['status'];
		}
	...
	public function doAddEvent($eventMap,$optsMap,$cover){
		...
		if( 1 == $has_friend  ){
			//我关注的人的ID
			$fids = M('follow')->field('fid')->where("uid={$eventMap['uid']} AND type='following'")->findAll();
			foreach($fids as $k=>&$v){
				$fids[$k] = $v['fid'];
			}
	...
	public function getEventContent( $eventId){
		...
		//获取发布者个人信息
		$result['admin'] = getUserBaseInfo($result['uid']);
		...
		$result['sdate'] = date("Y年m月d日 H:s",$result['sTime']);
		$result['edate'] = date("Y年m月d日 H:s",$result['eTime']);
		$result['explain'] = htmlspecialchars_decode($result['explain']);
		$result['intro'] = msubstr(real_strip_tags($result['explain']),0,50);
		$result = $this->appendContent( $result );
		//TODO
        $result['cover']     = getCover($result['coverId'],100,100);
		return $result;
	}
	...
	function getConfig($key=NULL){
		...
		($config['credit']      || $config['credit']===0) || $config['credit']=100;
    ...
    public function getHotList($limit=10){
    	$opts_ids  = self::factoryModel( 'opts' )->field('id')->where('isHot=1')->limit($limit)->findAll();
    	...
    	foreach($event_ids as &$v){
            ...
            $v['cover'] = getCover($v['coverId']);
			$v['url'] = $this->getEventUrl('/index/eventDetail',array('id'=>$v['id'],'uid'=>$v['uid']));
			$v['intro'] = msubstr(real_strip_tags($v['explain']),0,60);
			$v['sdate'] = getFormatDate($v['sTime']);
    	}
    	return $event_ids;
    }
	...
	//获取活动连接
	function getEventUrl($type='',$param){
		return U('event'.$type,$param);
	}
