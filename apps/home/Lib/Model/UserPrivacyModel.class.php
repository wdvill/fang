...
class UserPrivacyModel extends Model {
	...
	function dosave($data,$uid){
		...
		$this->query("REPLACE INTO {$this->tablePrefix}user_privacy (uid,`key`,`value`) VALUES ".implode(',', $sql));
	}
	...
	function setBlackList($mid,$type,$fid){
		if($type=='add'){
			...
			if( M('user_blacklist')->where($map)->count()==0 ){
				...
				M('follow')->where("(uid=$mid AND fid=$fid) OR (uid=$fid AND fid=$mid)")->delete(); //自动解除关系
				return true;
			}else{
				return false;
			}
