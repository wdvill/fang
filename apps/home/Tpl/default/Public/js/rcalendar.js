...
function rcalendar(text, mode, retfunction) { //文本域对象, 模式(dateonly,full), 选择日期后的事件函数(函数是新时间的Date对象)
	...
	if (!div_rcalendar) { //如果不存在，则初始化创建它
		...
		if (document.compatMode == "BackCompat" && navigator.userAgent.indexOf("MSIE") != -1) {
			...
			css += "#rtime {width:90px; height:20px; float:left; overflow:hidden;}";
			...
			css += "#rbtns {width:90px; margin-left:10px; height:20px; float:right; text-align:right; overflow:hidden;}";
		...
		str += '<div style="width:190px; height:20px; margin-top:4px; overflow:hidden;">';
		str += '  <div id="rtime">';
