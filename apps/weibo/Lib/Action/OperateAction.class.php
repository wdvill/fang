...
class OperateAction extends Action{
    ...
    function publish(){
        ...
        $id = $pWeibo ->publish( $this->mid , $data, 0 ,intval( $_POST['publish_type']) , h($_POST['publish_type_data']) ,$_POST['sync']);
        if( $id ){
    ...
    function transpond(){
    	...
    	if($_POST){
	        ...
	        if( $id = $pWeibo->transpond($this->mid,$post) ){
	        	$data = $pWeibo->getOneLocation($id);
				X('Credit')->setUserCredit($this->mid,'add_weibo')
						   ->setUserCredit($data['expend']['uid'],'add_weibo');
