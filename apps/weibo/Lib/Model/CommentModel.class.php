...
class CommentModel extends Model {
    ...
    function doaddcomment($uid, $post, $api=false)
    {
        ...
        if ( $comment_id = $this->addcomment( $data ) ){
			...
			$return['html'] =  '<div class="p10" id="comment_list_c_'.$comment_id.'">
									<div class="l wid12 tc"><a href="'.U('home/space/index',array('uid'=>$this->mid)).'" class="pic">
            		<img class="pic30" src="'.getUserFace($uid,'m').'" width="50" height="50"/></a></div>
									<div class="r wid86 rtit5">
										<p>'.getUserSpace($uid, 'fn', '', '{uname}').'<label>  刚刚</label></p>
										<span>'.formatComment($data['content'] ,true).'</span>
									</div>
								</div>
								<div class="cl"></div>
								<div class="border5 hei7"></div>
								<div class="cl"></div>';
            if( $post['transpond'] != 0 ){
    ...
    function getComment( $id, $limit = 10, $order = 'comment_id DESC' ){
		return $this->where("weibo_id={$id} AND isdel=0")->order($order)->findAll();
    ...
    function getCommentList($type='receive',$person='all',$uid) {
    	if ($type == 'send') { // 发出的评论
	    	if ($person == 'follow') {
	    		$map = "reply_uid IN (SELECT fid FROM {$this->tablePrefix}follow where uid={$uid})";
	    	}else if ($person=='other'){
	    		$map = "reply_uid NOT IN (SELECT fid FROM {$this->tablePrefix}follow where uid={$uid})";
	    	}else{
	    		$map = '1=1';
	    	}    		
    		...
    		if ($person == 'follow') {
	    		$map = "uid IN (SELECT fid FROM {$this->tablePrefix}follow where uid={$uid})";
	    	} else if ($person=='other') {
	    		$map = "uid NOT IN (SELECT fid FROM {$this->tablePrefix}follow where uid={$uid})";
	    	} else {
	    		$map = '1=1';
	    	}
