...
class WeiboApiModel extends WeiboModel
{
	...
	public function friends_timeline($uid, $since_id, $max_id, $count = 20, $page = 1)
	{
		...
		$weibo_ids = getSubByKey($list, 'weibo_id');
		foreach($list as $k => $v) {
			$v['favorited'] = isfavorited($v['weibo_id'], $uid, $weibo_ids);
			$result[$k]     = $this->getOneApi('', $v);
	    }
    	return $result;
	}
    ...
    function following($uid,$uname,$since_id,$max_id,$count=20,$page=1){
		...
		$map['uid']=$uid;
		$list = M('follow')->where($map)->limit($limit)->field('fid as uid,follow_id as id')->order('follow_id DESC')->findAll();
    ...
    function followers($uid,$uname,$since_id,$max_id,$count=20,$page=1){
		...
		$map['fid']  = $uid;
		$map['type'] = 0;
		$list = M('follow')->where($map)->limit($limit)->field('uid,follow_id as id')->order('follow_id DESC')->findAll();
    ...
    function searchUser($key,$mid,$since_id,$max_id,$count=20,$page=1){
    	...
    	foreach ($list as $k=>$v){
			...
			$list[$k]['followed_count'] 	= M('follow')->where('uid='.$v['uid'])->count();
			$list[$k]['followers_count']	= M('follow')->where('fid='.$v['uid'])->count();
