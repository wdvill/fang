...
//		dump(WEB_PUBLIC_PATH);
	...
	static private function checkUser() {
		...
		if (!service('Passport')->isLogged()) { // 未登陆
			// 邀请
			if (APP_NAME == 'home' && strtolower(MODULE_NAME) == 'index' && ACTION_NAME=='index' && isset($_REQUEST['invite']))
				redirect(U('home/public/register',array('invite'=>$_REQUEST['invite'])));
			
			if (APP_NAME == 'home' && strtolower(MODULE_NAME) == 'index' && ACTION_NAME=='index')
				redirect(U('home/public/register'));
				
			// 是否开启游客访问
			if (APP_NAME == 'home' && strtolower(MODULE_NAME) == 'space' && !$ts['site']['site_anonymous'])
				redirect(U('home/public/register'));
