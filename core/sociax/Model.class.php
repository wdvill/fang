...
class Model extends Think
{
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                $this->fields = F('_fields/'.$this->getTableName());
    ...
    public function flush() {
        ...
        // 2008-3-7 增加缓存开关控制
		// 2010-12-14 缓存文件名使用真实数据库表名，因为公共Model和应用Model有可能重名
        if(C('DB_FIELDS_CACHE'))
            // 永久缓存数据表信息
            F('_fields/'.$this->getTableName(),$this->fields);
    }
