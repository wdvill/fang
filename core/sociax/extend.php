...
function addStatistics(){
	include(CORE_PATH.'/sociax/Statistics.php');
	...
	$screen = "";
	//获取当前URL
	$currenturl = $statistics->getCurrentUrl();
	//获取网站
	$sitetype = 'juhuier';
	
	//检查是否支持wml格式文件
	if(!$statistics->check_wap())
	{
		$visittype = "web";
		$httpheader = $statistics->getHttpHeader();
		$ua = $statistics->getUA();
		$httpaccept = $statistics->getHttpAccept();
		$ip = $statistics->getIP();
		$browser = $statistics->browsers();
		$referer = $statistics->getReferer();
	} else {
		$visittype = "wap";
		$phonenumber = $statistics->getPhoneNumber();
		$httpheader = $statistics->getHttpHeader();
		$ua = $statistics->getUA();
		$phonetype = $statistics->getPhoneType();
		$screen = $statistics->getScreen();
		$httpaccept = $statistics->getHttpAccept();
		$ip = $statistics->getIP();
		$browser = $statistics->browsers();
		$referer = $statistics->getReferer();
	...
	//判断ip
	$iparr = array('221.174.25.27','123.112.72.120','220.181.126.124','122.49.11.25');
	if(!in_array($ip,$iparr) && !stristr($ua,'spider') && !stristr($ua,'Googlebot')){
		//将获取信息插入到数据库
		$data = array('visittype'=>$visittype,'sitetype'=>$sitetype,'phonenumber'=>$phonenumber,'httpheader'=>$httpheader,'ua'=>$ua,'phonetype'=>$phonetype,'httpaccept'=>$httpaccept,'ip'=>$ip,'browser'=>$browser,'time'=>$time,'referer'=>$referer,'currenturl'=>$currenturl,'screen'=>$screen);
...
function getUserCredit($uid){
	$res = M("credit_user")->where("uid={$uid}")->find();
	
	$res['gold'] = floatval($res['gold']);
...
function getNotifyCount($uid){
	//获取未读私信数量
	$message_count = M("message")->where("to_uid={$uid} AND is_read=0 AND to_delete_by=0")->field("message_id")->count();
...
//js弹窗提示
function handleErrorByJs($content,$url='-1'){
	header('Content-Type:text/html;charset=UTF-8');
	if($url=='-1'){
		echo "<script>alert('".$content."');history.back(-1);</script>";
	}elseif($url=='-2'){
		echo "<script>alert('".$content."');return false;</script>";
	}elseif($url=='close'){
		echo "<script>alert('".$content."');window.close();</script>";
	}else{
...
//判断用户是否为组织者
function isUserOrganizer($uid){
	return M("user_group_link")->where("uid=".$uid." AND user_group_id IN (1,4)")->count();
}

//处理日期
function getFormatDate($time){
	$time = $time ? $time :time();
	$return['date'] = date("Y年m月d日",$time);
	$return['mdate'] = date("m月d日",$time);
	
	$week = array('星期日','星期一','星期二','星期三','星期四','星期五','星期六');
	$getdate = getdate($time);
	$return['weekday'] = $week[intval($getdate['wday'])];
...
//主要用于js分页
function getPages($page,$total,$page_size){
	//获取总页数
	if($total){
		if($total < $page_size){
			$total_page = 1;
		...
		if($total%$page_size){
			$total_page = intval($total/$page_size)+1;
		}else{
	...
	if($total_page > 1){
		...
		if($page == 1){
			$return .= '<a href="'.$url.'&p='.($page+1).'" class="btn-next"><span>下一页</span></a>';
		}elseif($page == $total_page){
			$return .= '<a href="'.$url.'&p='.($page-1).'" class="btn-previous"><span>上一页</span></a>';
		}else{
			$return .= '<a href="'.$url.'&p='.($page-1).'" class="btn-previous"><span>上一页</span></a><a href="'.$url.'&p='.($page+1).'" class="btn-next"><span>下一页</span></a>';
		}
		$return .= "</div>";
	}else{
		$return = "";
...
//处理非法标签，只转义a标签
function TagStriplashes($content){
...
//返回解析的空间地址
function getUserSpace($uid,$class,$target,$text, $verified = true) {
	...
	if($t){
		if($t['1']=='uname'){
	 		...
	 		if ($verified) {
	 			$user_verified_icon = getUserVerifiedIcon($uid);
	 		}
	 	}else{
	...
	return "<a href='{$url}' class='{$class}' target='{$target}'>$text</a>" . $user_verified_icon;
}

//获取用户个人主页连接
function getUserUrl($uid,$type='index'){
	static $_USERINFO = array();
	if(!isset($_USERINFO[$uid])){
		$map['uid']   = $uid;
	...
	if($_USERINFO[$uid]['domain'] && !preg_match("/[^a-zA-Z0-9]/",$_USERINFO[$uid]['domain'])){
		$data['uid'] = $_USERINFO[$uid]['domain'];
	}
	$url = U('home/space/'.$type,$data);
	return $url;
}

//获取用户介绍
function getUserIntro($uid,$type){
	$map['uid']   = $uid;
	$intro = M('User')->where($map)->field('intro')->find();
	return htmlspecialchars_decode($intro['intro']);
}

//删除物品
function deleteThing($pic_id,$uid){
	if(!$uid) return 0;
	
	if(M('picture')->where("pic_id=$pic_id AND uid=$uid")->count()){
		if(M('picture')->where("pic_id=$pic_id")->delete()){
			return 1;
		}
	}else{
		return 0;
...
//分配邀请码
function sendinvite($uid,$count=0){
	$data['code'] = md5('yihong'.$uid.'yihong');
	$data['uid'] = $uid;
	$data['is_used'] = 0;
	$data['is_received'] = '1';
	if(M('invitecode')->add($data)){
		return $data['code'];
	}
}

//获取物品页面url
function getThingUrl($pic_id,$ptype){
	if($ptype){
		//$data['pt'] = $ptype;
		$url = "/list/".$ptype;
	}else{
	...
	//$url = U('home/User/things',$data);
	return $url;
}

//获取物品专辑页面url
function getAlbumUrl($album_id,$uid){
	if(!$album_id || !$uid) return '';

	$data['uid'] = $uid;
	$data['aid'] = $album_id;

	$url = U('home/Space/album',$data);
	return $url;
}

//外部链接跳转处理
function link_redirect($url,$type=''){
	if($type==1){
		return U('home/public/dlink',array('url'=>urlencode($url))).'&tp='.$type;
	}else{
		return U('home/public/dlink',array('url'=>urlencode($url)));
	}
}

function getUserVerifiedIcon($uid)
{
    $verified = M('user_verified')->field('info')->where("uid={$uid} AND verified='1'")->find();
    if ($verified) {
    	$html = "<img class='ts_icon' src=" . THEME_URL."/images/v_01.gif title=" . $verified['info'] . ">";
	}
	return $html;
}
...
function getUserInfo($uid,$uname,$mid,$status=false){
	...
	$data['visit_count'] = M("user_visited")->where("fid=".$user['uid'])->field("visited_id")->count();
	$data['friend_count']    = M("friend")->where("uid=".$user['uid']." OR friend_uid=".$user['uid']." AND status=1")->field("friend_id")->count();
	$data['follower_count']  = M("follow")->where("fid=".$user['uid'])->field("follow_id")->count();
	$data['following_count'] = M("follow")->where("uid=".$user['uid'])->field("follow_id")->count();
	$data['is_followed']     = getFollowState($mid,$user['uid']);
	//获取用户等级
	//$data['level'] = X("Credit")->getUserCreditLevel($uid);
	if($status){
...
function getUserBaseInfo($uid){
	...
	$user = M('user')->where("uid=".$uid)->field("email,uid,uname,position,company,domain,password")->find();
	if(!$user) return ;
	...
	$data['position']  = $user['position'];
	$data['company']   = $user['company'];
	$data['face']      = getUserFace($user['uid'],'m');
	$data['url']       = getUserUrl($user['uid']);
	$data['password']  = $user['password'];
	$data['domain']    = $user['domain'];
	//获取用户等级
...
function getUserLevel($uid){
	...
	$user = M('user')->where("uid={$uid}")->field("admin_level")->find();
	if(!$user) return ;
...
function getFollowState($uid,$fid){
	...
	if(M('follow')->where("(uid={$uid} AND fid={$fid} ) OR (uid={$fid} AND fid={$uid} )")->count()==2 ){
		return 'follow';
	}elseif( M('follow')->where("uid={$uid} AND fid={$fid} ")->count() ){
		return 'follow';
	}else{
		return 'unfollow';
	}
 ...
 */
function getUserFace($uid,$size='b',$sex){
	//获取用户性别
	if($uid && !$sex){
		$sex = M('user_detail')->where("uid=".$uid)->field("sex")->find();
		$sex = $sex['sex'];
	}
	$size = ($size)?$size:'m';
	if($size=='m'){
	...
	$userface = UPLOAD_PATH.'/avatar/'.$uid.'/'.$type;
	if(is_file($userface)){
		return __UPLOAD__."/avatar/$uid/$type?".time();
	}else{
		if($sex==1){
			$pic = THEME_URL."/images/avatar_".substr($type,0,-4).".gif?".time();
		}else{
			$pic = THEME_URL."/images/avatar_".substr($type,0,-4).".gif?".time();
		}
 ...
 * 获取用户个人照片
 */
function getUserImage($uid,$size='b',$sex){
	//获取用户性别
	if(!$sex){
		$sex = M('user_detail')->where("uid=".$uid)->field("sex")->find();
		$sex = $sex['sex'];
	...
	if(is_file($userface)){
		...
		if($sex==2){
			$pic = THEME_URL."/images/woman_pic".substr($type,0,-4).".gif?".time();
		}else{
			$pic = THEME_URL."/images/man_pic".substr($type,0,-4).".gif?".time();
		}
		return $pic;
	}
}
 ...
 * @return boolean
 */
function isUsernameAvailable($username, $uid = 0) {
	if ($uid == 0) {
		return ! M('user')->where('`username`="'.$username.'"')->find();
	}else {
		return ! M('user')->where('`email`="'.$username.'" AND `id` <> '.$uid)->find();
	}
}
...
function format($content,$url=false){
	if($url){
		$content = preg_replace('/((?:https?|mailto):\/\/(?:www\.)?(?:[a-zA-Z0-9][a-zA-Z0-9\-]*\.)?[a-zA-Z0-9][a-zA-Z0-9\-]*(?:\.[a-zA-Z]+)+(?:\:[0-9]*)?(?:\/[^\x{4e00}-\x{9fa5}\s<\'\"“”‘’]*)?)/u', '<a href="\1" target="_blank">\1</a>\2', $content);
	}
...
function formatComment($content,$url=false){
	if($url){
		$content = preg_replace('/((?:https?|mailto):\/\/(?:www\.)?(?:[a-zA-Z0-9][a-zA-Z0-9\-]*\.)?[a-zA-Z0-9][a-zA-Z0-9\-]*(?:\.[a-zA-Z0-9]+)+(?:\:[0-9]*)?(?:\/[^\x{4e00}-\x{9fa5}\s<\'\"“”‘’]*)?)/u', '<a href="\1" target="_blank">\1</a>\2', $content);
	}
...
function replaceEmot($data) {
	...
	if($info) {
		return preg_replace("/\[.+?\]/i","<img src='".__THEME__."/images/emotions/".$info['filename']."' />",$data[0]);
	}else {
		return $data[0];
	}
}
...
function getUserId($name) {
	...
	if ($info) {
		return getUserSpace($info['uid'], 'a_format', '_blank', "$name[0]", false);
	}else {
 ...
 * @return int
 */
function OnlineUserCheck($uid) {
	$time = time() - 15 * 60;
    $sql = "SELECT uid FROM " . C('DB_PREFIX') . "user_online WHERE uid = $uid and `ctime` > '$time'";
    $res = M('')->query($sql);
    return $res[0]['uid'];
}
...
//curl方法
function curls($url,$type='get',$postfields=''){
	$curl = curl_init();
