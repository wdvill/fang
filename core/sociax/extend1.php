...
function getFollowState($uid,$fid,$type=0) {
	...
	if ($type == 0) {
		...
		if (M('follow')->where("(uid=$uid AND fid=$fid AND type=0) OR (uid=$fid AND fid=$uid AND type=0)")->count() == 2) {
			return 'eachfollow';
		}else if ( M('follow')->where("uid=$uid AND fid=$fid AND type=0")->count()) {
			return 'havefollow';
		}else {
			return 'unfollow';
		}
		...
		$topic = M('follow')->field('a.topic_id')
									 ->table("{$db_prefix}weibo_topic a LEFT JOIN {$db_prefix}follow b ON b.fid=a.topic_id")
...
function getUserId($name) {
	...
	if ($info) {
		return getUserSpace($info['uid'], 'null', '', "$name[0]", false);
	}else {
