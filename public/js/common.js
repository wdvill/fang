...
ui = window.ui ||{
	...
	confirm:function(o,text){
		...
		text = text || '确定要做此项操作吗？';
		this.html = '<div id="yh_ui_confirm" class="yh_confirm"><span class="txt"></span><br><input type="button" value="确定"  class="btn_b mr5"><input type="button" value="取消"  class="btn_w"></div>';
		// 修改原因: yh_ui_confirm .btn_b按钮会重复提交
		//if( $('#yh_ui_confirm').html()==null ){
			$('body').append(this.html);
		//}
		var position = $(o).offset();
		$('#yh_ui_confirm').css({"top":position.top+"px","left":position.left-($("#yh_ui_confirm").width()/2)+"px","display":"none"});
		$("#yh_ui_confirm .txt").html(text);
		$('#yh_ui_confirm').fadeIn("fast");
		$("#yh_ui_confirm .btn_w").one('click',function(){
			$('#yh_ui_confirm').fadeOut("fast");
			$('#yh_ui_confirm').remove();
		});
		$("#yh_ui_confirm .btn_b").one('click',function(){
			$('#yh_ui_confirm').fadeOut("fast");
			$('#yh_ui_confirm').remove();
			eval(callback);
		});
	...
	emotions:function(o){
		...
		this.showContent = function(data){  //显示表情内容
			...
			$.each(data,function(i,n){
				content+='<a href="javascript:void(0)" onclick="ui.emotions_c(\''+n.emotion+'\',\''+target_set+'\')"><img src="'+ _THEME_ +'/images/emotions/'+ n.filename +'" /></a>';
			});
	...
	getarea1:function(prefix,init_style,init_p,init_c){
		var style = (init_style)?'class="'+init_style+'"':'';
		// _PUBLIC_+'/js/area.js'
		$.getJSON(U('home/Public/getArea'), function(json){
			json = json.provinces;
